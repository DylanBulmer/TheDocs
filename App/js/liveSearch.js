...
function liveSearch() {
	...
	xhttp.onreadystatechange = function () {
		if (this.readyState == 4 && this.status == 200) {
			...
			tab.setAttribute("style", "top: 37px;")
			...
			if (result.length == 0) {
				...
				for (i = 0; i < result.length; i++) {
					...
					tab.addEventListener("click", function (e) {
						...
						console.log(top, t.getBoundingClientRect().top - w.getBoundingClientRect().top, top + (t.getBoundingClientRect().top - w.getBoundingClientRect().top) + "px")
